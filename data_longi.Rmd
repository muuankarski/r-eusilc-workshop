...
Merging raw panel data
-----------------------------------

Here we are only interested in individual level events and consequences. So we are reading all panel data personal files from 2008, 2009 and 2010 datasets in R and, we write the merged datasets in local file (folder `demo/year`). [r.eusilc](https://github.com/muuankarski/r.eusilc)-package
...
```{rloaddata, cache=TRUE, warning=FALSE, message=FALSE, eval=FALSE}
...
```{rsubsetting_simple, cache=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{rsubsetting_compilated, cache=TRUE, message=FALSE, warning=FALSE}
...
```{rplot1, cache=TRUE, message=FALSE, warning=FALSE}
...
```{rsubsetrecodeexitpension, cache=TRUE, message=FALSE, warning=FALSE}
library(reshape2)
...
```{rsubsetenterpensionhistogram, cache=TRUE, message=FALSE, warning=FALSE}
...
```{rsubsetfigpension1, fig.height=10, cache=TRUE, message=FALSE, warning=FALSE}
library(ggplot2)
ggplot(data=datTp, 
...
```{rsubsetfigpension2, cache=TRUE, message=FALSE, warning=FALSE}
library(RCurl)
...
```{rsubsetpensionplotAbsobox, cache=TRUE, message=FALSE, warning=FALSE}
library(ggplot2)
...
```{rsubsetpensionplotAbso, fig.height=15, cache=TRUE, message=FALSE, warning=FALSE}
library(ggplot2)
